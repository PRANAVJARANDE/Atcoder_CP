...
ll helper(ll in,ll k,ll turn,vvl&adj,string&s,vector<vvl>&dp)
{
    if(k==0)
    {
        if(s[in]=='A')return 1;
    ...
    if(dp[in][k][turn]!=-1)return dp[in][k][turn];
    for(auto x:adj[in])
    {
        ll res=helper(x,k-1,1-turn,adj,s,dp);
        if(turn==0)
        {
            //alice turn 
            if(res==1)return dp[in][k][turn]=1;
        }
        else
        {
            //bob turn
            if(res==0)return dp[in][k][turn]=0;
        }
    }
    return dp[in][k][turn]=turn;
}

void solve()
{
    ll n,sz,k;cin>>n>>sz>>k;
    string s;cin>>s;
    vvl adj(n);
    while(sz--)
    {
    ...
    vector<vvl>dp(n,vvl((2*k)+1,vll(2,-1)));
    ll ans=helper(0,2*k,0,adj,s,dp);
