...
void solve()
{
    ...
    map<char,vector<pair<ll,ll>>>tele;
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
    ...
    vvl dis(n,vll(m,INF));
    dis[0][0]=0;
    priority_queue<pair<ll,pair<ll,ll>>,vector<pair<ll,pair<ll,ll>>>,greater<pair<ll,pair<ll,ll>>>>q1;
    q1.push({0,{0,0}});
    ...
    while(!q1.empty())
    {
        ...
        for(int z=0;z<4;z++)
        {
            ll nx=x+dx[z];
            ll ny=y+dy[z];
            if(issafe(nx,ny) && a[nx][ny]!='#' && dis[nx][ny]>1+dis[x][y])
            {
                dis[nx][ny]=1+dis[x][y];
