...
void build(int node, int i, int j, vector<ll>& tree, vector<ll>& base) 
{
    if (i == j) {
        tree[node] = base[i];
        return;
    ...
    int mid = (i + j) / 2;
    build(2 * node + 1, i, mid, tree, base);
    build(2 * node + 2, mid + 1, j, tree, base);
...
ll val;
ll dfs(ll in,ll par,vvl &adj,vll&idx,vll&cnt)
{
    idx[in]=val++;
    ll s=1;
    for(auto x:adj[in])
...
void solve()
{
    ...
    while(q--)
    {
        ...
        else
        {
            ...
            ll s1=query(0,0,n-1,tree,l,r);
            ll s2=query(0,0,n-1,tree,0,n-1)-s1;
            ll diff=abs(s1-s2);
            cout<<diff<<endl;
        }
    }
