...
ll helper(ll in,ll h,vll&a,vll&b,vvl&dp)
{
    if(h<0)return 1e18;
    if(in<0)return 0;
    if(dp[in][h]!=-1)return dp[in][h];
    ll op1=helper(in-1,h-a[in],a,b,dp);
    ll op2=b[in]+helper(in-1,h,a,b,dp);
    return  dp[in][h]=min(op1,op2);    
}
