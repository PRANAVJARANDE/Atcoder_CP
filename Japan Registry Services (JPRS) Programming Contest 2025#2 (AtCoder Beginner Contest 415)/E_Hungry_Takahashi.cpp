...
bool check(ll mid,vvl&a)
{
    vvl dp(n,vll(m,LLONG_MIN));
    dp[0][0]=mid+a[0][0];
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            ...
            if(j-1>=0 && dp[i][j-1]!=LLONG_MIN)op1=dp[i][j-1]+a[i][j];
            if(i-1>=0 && dp[i-1][j]!=LLONG_MIN)op2=dp[i-1][j]+a[i][j];
            dp[i][j]=max(op1,op2);
            if(dp[i][j]<0)dp[i][j]=LLONG_MIN;
        }
    }
