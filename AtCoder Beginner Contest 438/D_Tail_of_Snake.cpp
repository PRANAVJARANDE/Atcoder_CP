...
ll n;
ll helper(ll in,ll cnt,vll&a,vll&b,vll&c,vvl&dp)
{
    if(in>=n)
    {
        if(cnt>=2)return 0;
    ...
    if(cnt==0)
    {
        ll op1=a[in]+helper(in+1,0,a,b,c,dp);
        ll op2=-INF;
        if(in!=0)op2=b[in]+helper(in+1,1,a,b,c,dp);
        ans=max(op1,op2);
    }
    else if(cnt==1)
    {
        ll op1=b[in]+helper(in+1,1,a,b,c,dp);
        ll op2=c[in]+helper(in+1,2,a,b,c,dp);
        ans=max(op1,op2);
    }
    else 
    {
        ll op1=c[in]+helper(in+1,2,a,b,c,dp);
        ans=op1;
    }
    return dp[in][cnt]=ans;
}
