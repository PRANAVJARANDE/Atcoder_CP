...
void solve()
{
    ...
    {
        psum[i]=b[i]%MOD;
        if(i-1>=0)psum[i]=((psum[i-1]%MOD) + (psum[i]%MOD))%MOD;
    }
    ...
    {
        ll in=upb(b,x);
        ans=(ans+(in%MOD)*(x%MOD))%MOD;
        ans=(ans+psum[m-1]%MOD)%MOD;
        ans=(ans-(( (m-in + MOD)%MOD+MOD)%MOD*(x%MOD))%MOD+MOD)%MOD;
        if(in-1>=0)
        ans=(ans-(2*(psum[in-1]%MOD))%MOD+MOD)%MOD;
    }
    cout<<ans<<endl;
}
